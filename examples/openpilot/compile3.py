...
from tinygrad.dtype import dtypes, _from_np_dtype
...
def process_video_frame(frame, input_shapes, input_types):
  ...
  for k, shp in sorted(input_shapes.items()):
    if k not in inputs:
      ...
      # 使用直接的NumPy数据类型，而不是通过_from_np_dtype转换
      inputs[k] = Tensor(np.zeros(shp, dtype=np.float32), device="GPU").realize()
...
def parse_supercombo_output(output_tensor):
  ...
  # 检查输出张量的形状和内容
  print("\n===== 驾驶模型输出解析结果 =====\n")
  print(f"output:\n  形状: {output_tensor.shape}\n  数据统计: 最小值={output_tensor.min():.4f}, 最大值={output_tensor.max():.4f}, 均值={output_tensor.mean():.4f}")
  # 确保输出张量是NumPy数组
    ...
    if model_file.endswith(".onnx"):
      try:
        ...
        if has_onnx:
          ...
          # 尝试编译ONNX模型为可执行函数
          try:
            from tinygrad.nn.state import safe_load, safe_save, get_state_dict
            from tinygrad.nn.onnx import safe_load_onnx
            print("尝试使用tinygrad编译ONNX模型...")
            
            # 使用tinygrad的ONNX加载器编译模型
            model_data = safe_load_onnx(onnx_model)
            print("ONNX模型编译成功")
          except Exception as e:
            print(f"编译ONNX模型时出错: {e}")
            # 如果编译失败，仍然保存原始模型用于获取输入形状
            model_data = onnx_model
      ...
      try:
        ...
        try:
          ...
          if demo_mode or model_data is None:
            ...
            try:
              ...
              print("成功导入Context从tinygrad.runtime.ops_backprop")
            except ImportError:
              pass
              
            # 尝试导入方式2: 新版可能的路径
            if not context_imported:
              try:
                from tinygrad.runtime.lib import Context
                context_imported = True
                print("成功导入Context从tinygrad.runtime.lib")
              except ImportError:
                pass
            ...
            if not context_imported:
              try:
                from tinygrad.engine.jit import Context
                context_imported = True
                print("成功导入Context从tinygrad.engine.jit")
              except ImportError:
                pass
            
            # 无论是否成功导入Context，都尝试运行模型
            try:
              if context_imported:
                ...
                # 如果无法导入Context，直接尝试运行模型
                print("无法导入Context，直接尝试运行模型")
                if hasattr(model_data, 'run'):
                  output = model_data.run(**inputs)
                elif callable(model_data):
                  output = model_data(**inputs)
        ...
        if demo_mode or model_data is None:
          ...
          try:
            ...
            # 无论原始解析是否成功，都采用安全的解析方式
            try:
              # 检查输出张量大小是否满足最低要求
              if output_np.size >= 1000:
                # 尝试安全地reshape数组
                try:
                  # 解析车道线
                  lane_lines_size = 4 * 33 * 2  # 4条车道线，33个点，每个点2个坐标
                  lane_lines_data = output_np.flatten()[:lane_lines_size].reshape(1, 4, 33, 2)
                  
                  # 解析路沿
                  road_edges_size = 2 * 33 * 2  # 2条路沿，33个点，每个点2个坐标
                  road_edges_data = output_np.flatten()[lane_lines_size:lane_lines_size+road_edges_size].reshape(1, 2, 33, 2)
                  
                  # 其他数据使用默认值
                  parsed_outputs = {
                    'lane_lines': lane_lines_data,
                    'road_edges': road_edges_data,
                  ...
                  print(f"警告: 输出解析过程中出错: {reshape_err}")
                  # 解析失败时使用模拟数据
                  parsed_outputs = {
