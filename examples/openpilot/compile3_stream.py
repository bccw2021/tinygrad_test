...
def process_video_frame(frame, input_shapes, input_types):
  ...
  if 'input_imgs' in input_shapes:
    ...
    # 如果需要多帧输入（如channels > 3），复制当前帧填充
    if channels > 3:
      # 假设每3个通道代表一帧RGB图像
      frames_needed = channels // 3
      stacked_frames = np.zeros((height, width, channels), dtype=np.float32)
      
      # 复制当前帧到所有位置
      for i in range(frames_needed):
        stacked_frames[:, :, i*3:(i+1)*3] = normalized_frame
      
      # 调整形状为(batch_size, channels, height, width)
      input_img = np.transpose(stacked_frames, (2, 0, 1))[np.newaxis, ...]
    else:
      # 单帧情况，调整形状为(batch_size, channels, height, width)
      input_img = np.transpose(normalized_frame, (2, 0, 1))[np.newaxis, ...]
  ...
  # 为其他输入生成随机数据
  Tensor.manual_seed(int(time.time() * 1000) % 10000)  # 使用当前时间作为随机种子
  
  for k, shp in sorted(input_shapes.items()):
    if k not in ['input_imgs', 'big_input_imgs'] and k in input_shapes:  # 跳过已处理的图像输入
      # 生成随机张量并保存到NumPy数组
      tensor = Tensor.randn(*shp, dtype=_from_np_dtype(input_types[k])).mul(0.1).realize()  # 使用较小的值
      inputs[k] = Tensor(tensor.numpy(), device="NPY").realize()
